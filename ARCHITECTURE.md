...
| Guided-flow selection logging | [app/api/log-selection/route.ts](app/api/log-selection/route.ts) → [lib/analytics.ts](lib/analytics.ts) `logSelection()` | `POST`. Household size / device count / usage profile / service-type-filter choices happen entirely client-side (no `/api/chat` call necessarily follows), so they're posted here as a fire-and-forget update to the most recent `chat_logs` row for that session. |
| Plan/recommendation helpers | [lib/plan-utils.ts](lib/plan-utils.ts) | Shared `Plan`/`PlanGroups` types, `HOUSEHOLD_SIZE_OPTIONS`/`DEVICE_COUNT_OPTIONS`/`USAGE_PROFILE_OPTIONS`, and `recommendPlan()` — the algorithm behind the guided "get a personalized recommendation" flow (household size → number of devices → usage profile, in that order). `recommendPlan()` takes the *higher* of (a) a household-size/usage-derived baseline and (b) a fixed minimum-download-speed floor per device-count bracket (10 Mbps for 1-2 devices up to 2 Gbps for 30+, per an FCC-style broadband speed guidance table) — the device-count floor guarantees a baseline the guidance table considers necessary, while household size/usage can still push the requirement higher. Also imported by the admin dashboard to label its household/devices/usage charts. |
...
├── plan-utils.ts        # Plan/PlanGroups types, household/device/usage options, recommendPlan()
