      ...
      <AlertDialog.Portal>
        ...
        <AlertDialog.Popup finalFocus={() => { onClosed(); return false; }} className="fixed top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-background text-foreground p-6 shadow-lg">
          <div className="flex flex-col gap-3">
            <AlertDialog.Title className="text-lg font-semibold">{t("Clear this client's conversation?")}</AlertDialog.Title>
            <AlertDialog.Description className="text-sm leading-relaxed text-muted-foreground">
              {t('This clears the conversation, draft, address, answers, and results from this screen.')} {t(action === 'home' ? 'Home will show the three starting options.' : 'New client will restart your current workflow.')}
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={onConfirm}>{t('Clear and continue')}</Button>
            </div>
          </div>
        </AlertDialog.Popup>
      </AlertDialog.Portal>
